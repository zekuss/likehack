...
<? if (!empty($arResult['FAVS'])): ?>
    ...
    <? foreach ($arResult['FAVS'] as $ID => $arItem):
        ?>
        <div class="asd_fav_item col-sm-12 col-md-12  bg-container">
            <? if (strlen($arItem['PREVIEW_PICTURE_RESIZED']['src'])) { ?>
            <div class="search-preview">
                <a href="<?= $arItem['DETAIL_PAGE_URL'] ?>">
                    <img class=" img-responsive img-polaroid" src="<?= $arItem['PREVIEW_PICTURE_RESIZED']['src'] ?>" alt=""/></a><? } ?>
            ...
            <? if ($arResult['CAN_EDIT'] == 'Y'): ?>
                <div class="asd_fav_menu">
                    <? if ($arParams['ALLOW_MOVED'] == 'Y'): ?>
                        <a href="#" class="asd_fav_move"
                           id="asd_fm_<?= $ID ?>"><?= GetMessage('ASD_TPL_FAV_MOVE') ?></a>
