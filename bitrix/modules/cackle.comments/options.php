...
//$_SERVER['HTTP_HOST']=CMainPage::GetSiteByHost();
...
{
    global $cackle_api;
    return $cackle_api->cackle_get_param('cackle_activated_'.$_SERVER['HTTP_HOST']);
}
    ...
    if (!empty($_GET['cf_action'])) {
        switch ($_GET['cf_action']) {
            ...
            case 'import_comments':
                if ($cackle_api->cackle_get_param("last_comment_".$_SERVER['HTTP_HOST'])) {
                    $cackle_api->db_connect("delete from " . PREFIX . "_comments where user_agent like 'Cackle:%%'",false);
                }
                $cackle_api->cackle_set_param("last_comment_".$_SERVER['HTTP_HOST'], 0);
                $cackle_api->cackle_set_param("last_modified_".$_SERVER['HTTP_HOST'], 0);
...
<div class="accordion">
    ...
    margin: 1em 0;"><?php print_r(GetMessage("CACKLE_SETTINGS"));  ?></h3>
                ...
                <p><?php print_r(GetMessage("CACKLE_PLEASE"));  ?>
                    , <a target="_blank"
                         href="http://admin.cackle.me/account/register?lang=ru"><?php print_r(GetMessage("CACKLE_CLICK"));  ?></a> <?php print_r(GetMessage("CACKLE_CLICK_FOR"));  ?>
                    Site ID, Account API Key, Site API Key. </p>
                <ul class="list">
                    <li>
                        <label>Cackle SiteId: </label>
                        <input id="siteId" name="siteId" class="input-number"
                               value="<?php echo $cackle_api->cackle_get_param('site_id_'.$_SERVER['HTTP_HOST']); ?>"/>
                    </li>
                    <li>
                        <label>Cackle Account API Key: </label>
                        <input id="accountApiKey" style="width: 550px;" name="accountApiKey" class="input-number"
                               value="<?php echo $cackle_api->cackle_get_param('account_api_'.$_SERVER['HTTP_HOST']); ?>"/>
                    </li>
                    <li>
                        <label>Cackle Site API Key</label>
                        <input id="siteApiKey" style="width: 550px;" name="siteApiKey" class="input-number"
                               value="<?php echo $cackle_api->cackle_get_param('site_api_'.$_SERVER['HTTP_HOST']); ?>"/>
                    </li>
                    ...
                    if (!empty($_POST)) {
                        ...
                        if (key_validate($_POST['siteId'], $_POST['siteApiKey'], $_POST['accountApiKey'])) {
                            ...
                            COption::SetOptionString("cackle.comments", "site_id_".$_SERVER['HTTP_HOST'], $_POST['siteId']);
                            COption::SetOptionString("cackle.comments", "site_api_".$_SERVER['HTTP_HOST'], $_POST['siteApiKey']);
                            COption::SetOptionString("cackle.comments", "account_api_".$_SERVER['HTTP_HOST'], $_POST['accountApiKey']);
                            COption::SetOptionString("cackle.comments", "cackle_sso_".$_SERVER['HTTP_HOST'], $_POST['enable_sso']);
                            COption::SetOptionString("cackle.comments", "cackle_encoding_".$_SERVER['HTTP_HOST'], ($_POST['enable_encoding']== 1) ? 1 :0);
                            COption::SetOptionString("cackle.comments", "cackle_activated_".$_SERVER['HTTP_HOST'], true);

                        } else {
                            echo '<span style="color:#ff0000"><p>'; print_r(GetMessage("CACKLE_ACTIVATION_ERROR"));  echo '</p></span>';
                        }
                    } else {
                        if (cackle_activate()) {
                            echo '<span style="color:green;font-weight:bold;">'; print_r(GetMessage("CACKLE_ACTIVATION_SUCCESS"));  echo '</span>';
                    ...
                    <li>
                        <?php echo "Single Sign On:"?> <input type="checkbox" value="1"
                                                              name="enable_sso" <?php if ($cackle_api->cackle_get_param('cackle_sso_'.$_SERVER['HTTP_HOST']) == 1): ?>
                                                              checked="checked" <?php endif;?>/>
                        <?php  print_r(GetMessage("CACKLE_SSO"));?> <a href="http://admin.cackle.me/pricing" title="See details about Pro Account">prices</a>.

                    </li>
                    <li>
                        <?php echo "Database encoding"?> <input type="checkbox" value="1"
                                                                name="enable_encoding" <?php if ($cackle_api->cackle_get_param('cackle_encoding_'.$_SERVER['HTTP_HOST']) == 1): ?>
                                                                checked="checked" <?php endif;?>/>
