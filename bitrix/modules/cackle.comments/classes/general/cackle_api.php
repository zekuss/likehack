...
$cackle_site_id = COption::GetOptionString("cackle.comments", "site_id_".$_SERVER['HTTP_HOST'], $_POST['siteId']);
$cackle_site_name = CMainPage::GetSiteByHost();
$cackle_account_api = COption::GetOptionString("cackle.comments", "account_api_".$_SERVER['HTTP_HOST'], $_POST['accountApiKey']);
$cackle_sso = COption::GetOptionString("cackle.comments", "cackle_sso_".$_SERVER['HTTP_HOST'], $_POST['enable_sso']);
$cackle_encoding = COption::GetOptionString("cackle.comments", "cackle_encoding_".$_SERVER['HTTP_HOST'], $_POST['enable_encoding']);
$cackle_activated = COption::GetOptionString("cackle.comments", "cackle_activated_".$_SERVER['HTTP_HOST'], $_POST['cackle_activated']);
...
class CackleAPI{
    ...
    function db_connect($query,$return=true,$list=false){
        ...
        if ($this->cackle_get_param("cackle_encoding_".$_SERVER['HTTP_HOST']) == 1){
    ...
    function import_wordpress_comments(&$wxr, $timestamp, $eof) {
        ...
        $postdata = http_build_query(
                array(
                    'siteId' =>$this->cackle_get_param("site_id_".$_SERVER['HTTP_HOST']),
                    'accountApiKey' => $this->cackle_get_param("account_api_".$_SERVER['HTTP_HOST']),
                    'siteApiKey' => $this->cackle_get_param("site_api_".$_SERVER['HTTP_HOST']),
